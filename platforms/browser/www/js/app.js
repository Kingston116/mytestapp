(function() {
            ...
            document.getElementById("refreshnews").addEventListener('click', function (e) {
                ...
                .then((data) => {
                    ...
                    for(var audiocomment in current_audio_list){
                        commentpanel += '<li data-role="fieldcontain" data-role="list-divider" style="display:flex;"><div style="width:50%;padding-top: 4%;"> <a style="font-size: smaller;"  href="#">'+current_audio_list[audiocomment]["time"]+' </a></div><audio id="player-'+audiocomment+'" style="width:50%" src="http://ec2-3-10-169-78.eu-west-2.compute.amazonaws.com/upload/upload/'+key+"/"+current_audio_list[audiocomment]["filename"]+'" controls controlsList="nodownload"></audio></li><hr>';
                    }
                    commentpanel+="</ul>"
    ...
    var panel = '<div id="refreshhome" class="circle-div" style="z-index: 1;text-align: center;font-size: x-large;transform: rotate(90deg);color: white;"><p style="margin: 1px;">↻</p></div><input type="hidden" id="location" /><ul class="ui-listview ui-listview-inset ui-corner-all ui-shadow" data-role="listview" data-inset="true" style="padding-left: 0px;text-align: center;">';
    ...
    .then((data) => {
        for (i = 0; i < Object.keys(data.news).length; i++) {
            panel += '<li class="news" data-transition="pop" id="'+Object.keys(data.news)[i]+'"><a href="#" style="color: #082aa2;font-size:small"><img style="width: 90%;height:200px;object-fit:cover;padding-top: 10px;padding-right: 10px;padding-left: 10px;" src="'+data.news[Object.keys(data.news)[i]].image_path+'"/>';
            panel += '<p><b>'+Object.keys(data.news)[i]+'</b></p></a></li><hr>';
        }
        panel += "</ul>"
    ...
    function load_news(element){
      ...
      key = element.target.parentElement.children[1].children[0].innerText;
      window.location.href = "#news";
      $("#recordedComment").hide();
      fetch("http://ec2-3-10-169-78.eu-west-2.compute.amazonaws.com/detailednews/"+key).then((response) => response.json())
        .then((data) => {
