(function() {
    ...
    .then((data) => {
        for (i = 0; i < Object.keys(data.news).length; i++) {
            panel += '<li id="'+Object.keys(data.news)[i]+'"><img class="cimg" src="https://media-exp1.licdn.com/dms/image/C560BAQEC71ut5ITILQ/company-logo_200_200/0?e=2159024400&v=beta&t=0lqcMDSFycbaGBEmyuQu9EUHNgElUbHHaxreFDeJ6HE"/>';
            panel += '<div class="cinner"><p><b>'+Object.keys(data.news)[i]+'</b></p><p>'+data.news[Object.keys(data.news)[i]]+'</p><div style="clear:both"></div></li>';
        }
        panel += "</ul>"
    ...
    function load_news(element){
      alert(element.target.attributes.id.value)
